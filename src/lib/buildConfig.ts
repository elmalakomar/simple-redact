...
const buildFieldConfig = (splittedField: string[], result: ConfigRedact, index = 0) => {
  ...
  if(!simpleFieldConfig) {
    ...
    buildFieldConfig(splittedField, result[configIndex].data as ConfigRedact, index + 1)
    return
  }
  buildFieldConfig(splittedField, simpleFieldConfig.data as ConfigRedact, index +1)
  return
}
